...
- 入口は水槽詳細の設備スロット列の右端の絵ボタン（assets/icons/upgrade.png）だけ。ヘッダーとショップの一覧はオーナー指示で撤去。確認モーダル→ confirm → coinFx(−価格) → 水槽が一瞬ふくらむ .tank-pop

## 出荷画面（2026-09-24）
- 文字は濃く（.sell-list で小文字を var(--ink) 85%・12.5px）。オーナーから「見にくい」と指摘
- チェックは画面を作り直さず、その場で .cb と下部の合計だけ更新（作り直すとスクロールが先頭に戻るため）
- 色の絞り込み（すべて/赤/青/黄/黒/緑/透明/隠し色）と並び順（値段が高い順/色ごと/★が高い順）。状態は sellHue / sellSort

## 段階(tier)の決まり（2026-09-24 修正）
- tierOf: 透明・隠し色（HIDDEN_HUES）は縞や輝の遺伝子を持っていても常に★1。図鑑キー・価格と揃えるため。以前は透明の縞持ちが★4扱いになり出荷時に「貴重」と出ていた
- 保存済みの tier は state.migrate で読み込み時に計算し直す
- 透明・隠し色で pat==='s' の個体は「縞の遺伝子あり」チップを出す（色付きと掛けると縞が出る親として価値がある）

## 全体点検（2026-09-24）
- test/fuzz.test.js: ランダム操作を30日分回して不変条件（エビは水槽/遠征/バケツのどれか1つ、tankId一致、資金≥0、ゲージ範囲、隠し色は★1、セーブ往復）を検査。ルールを変えたら必ず通す
- 直したもの: 透明の縞持ちが★4扱い / 「どの色でも」の客が渡せない匹数を要求 / 見守りのヒントを実際の判定値（汚れ40%・餌「十分」）に合わせた

## 個体カードの切り替え（2026-09-25）
- カード上部に同じ水槽のエビの横スクロール一覧（.sheet-strip、現在の個体を強調・自動で中央へ）と ‹ › ボタン、カード本体の左右スワイプ（60px以上・横優位）で次/前へ。水槽内でタップしにくいという指摘への対応

## 画面を作り直すときの注意
- 一覧で選び直すたびに innerHTML を作り直す画面は、必ずスクロール位置を保つこと（出荷はその場更新、川へ送るは keepScroll で復元）。「先頭に戻る」はオーナーが2回指摘した
